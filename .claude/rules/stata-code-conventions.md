...
## 0. Running Stata in this project (batch mode)

This project has **no `stata` on `PATH`** and does **not** use `stata-mcp`. Claude runs Stata in **batch mode** via the console binary and reads the resulting log:

```bash
STATA=/Applications/StataNow/StataSE.app/Contents/MacOS/stata-se
"$STATA" -b do scripts/stata/99_run_all.do     # full pipeline
"$STATA" -b do scripts/stata/01_clean.do       # single step
```

`-b` writes a plain-text `<dofile>.log` beside the invocation; **always read it back** to verify the run (grep for `r(` error codes, confirm the final state). Prefer routing each script's own log into git-ignored `scripts/stata/_outputs/` via `log using` (see §1) — and never commit a log that printed microdata (see [`data-confidentiality.md`](data-confidentiality.md)). Note this is **StataSE**, so matrix / variable limits are SE-tier — prefer `reghdfe` over `areg`/`xi:` for high-dimensional fixed effects.
