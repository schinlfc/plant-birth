...
clear
import delimited "${directory}cadmium_distance_matches.csv"
tostring monitor_id, gen(monitor)
rename plant_code plantid
save "${directory}matched_cadmium.dta", replace
clear 

// Import Improve Data\

import delimited "${directory}cadmium_IMPROVE_EPA.csv"
rename countyfips fips
rename val arithmeticmean
...
save "${directory}cadmium_improve.dta", replace
...
merge m:1 monitor_id using "${directory}matched_cadmium.dta"
drop _merge
...
xtset monitor_id date, daily

// Identify Phaseout
sort monitor_id
by monitor_id: egen min_capacity = min(plant_capacity)
by monitor_id: egen max_capacity = max(plant_capacity)

// Restrict (Discover Dyads with Complete Phaseout, Long Monitor Series, and within 10km)
gen difference = max_year-min_year
tabstat difference_year monitor_id1 if min_capa==0 & distance<=10 , stat(max) by(plantid)
